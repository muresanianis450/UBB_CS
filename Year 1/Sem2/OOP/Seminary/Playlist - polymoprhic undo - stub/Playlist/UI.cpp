...
void UI::updateSongsRepo() {
	cout << "Enter the artist of the song to update: ";
	std::string artist;
	getline(cin, artist);
	...
	cout << "Enter the new YouTube link: ";
	std::string link;
	getline(cin, link);

	try {
		this->serv.updateSong(artist, title, minutes, seconds, link);
...
void UI::redoRepository() {
	try {
		this->serv.redo();
		cout << "Redo operation successful!" << endl;
	} catch (const std::exception& e) {
		cout << "Redo failed: " << e.what() << endl;
	}
}
