...
void testJumpBackward() {
	...
	SMMIterator it(smm);
	it.first();
	it.next(); // Move to the second value of key 1
	it.jumpBackward(1); // Jump back 1 step
	assert(it.getCurrent() == std::make_pair(1, 10)); // Should be back to the first value

	it.jumpBackward(1); // Jump back to an invalid state
	assert(!it.valid()); // Iterator should now be invalid

	try {
		it.jumpBackward(1); // Attempt to jump backward on an invalid iterator
...
void testTrimValues() {
	SortedMultiMap m([](TKey a, TKey b) { return a < b; });
	...
	// 1. No trimming needed if maxSize is larger than current values
	assert(m.trimValues(5) == 0);

	// 2. Trimming to 2 values for key 1 (should remove 2 values)
	m.add(1, 50); // Now key 1 has 5 values
	assert(m.trimValues(2) == 3);

	// 3. After trimming, key 1 should have only 2 values
	assert(m.search(1).size() == 2);

	// 4. Key 2 should remain unchanged (has less than maxSize values)
	assert(m.search(2).size() == 2);

	// 5. Trimming with maxSize 0 should remove all values for keys with values
	assert(m.trimValues(0) == 4); // 2 from key 1, 2 from key 2
}